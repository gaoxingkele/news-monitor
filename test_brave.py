"""Test: Brave Search only, for Uruguay.
...
async def main():
    ...
    print(f"Source    : Brave Search only")

    api_key = os.environ.get("BRAVEAPI", "")
    bs = BraveSearch({"api_key": api_key}, overseas_proxy)

    print(f"\n[Brave] fetching {len(query_groups)} query groups...")
    all_arts: list[NewsArticle] = []
    for i, kw_list in enumerate(query_groups):
        result = await bs.fetch_articles(
            keywords=kw_list,
        ...
        await asyncio.sleep(1.1)  # Brave free tier: 1 req/sec
