"""Fetch Colombia topic: newsdata_io only, two scopes (targeted + global), dedup."""
...
async def main() -> None:
    ...
    topics = [t for t in all_topics if "哥倫" in t.get("name", "") or "哥伦" in t.get("name", "")]
    if not topics:
        print("Colombia topic not found"); return
    topic = topics[0]
    ...
    targeted          = local_countries + watch_countries   # e.g. [co, tw, cn, us]
    ...
    # Scope 1: targeted countries (co + tw + cn + us)
