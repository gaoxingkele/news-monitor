"""Render GetOilFromChina v1.0 FINAL as formatted DOCX
- Body: FangSong 四号 = 14 pt
- Headings: FangSong bold at 22/15/14/12 pt
- Tables: FangSong 五号 = 10.5 pt
- References: FangSong 小五 = 9 pt with hanging indent
- Bold markers **...** preserved
- In-text citations [n] / [n,m] / [n-m] rendered as superscript
"""
...
src = os.path.join(os.path.dirname(os.path.abspath(__file__)), "04_report_v2.0_situation_report.md")
dst = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                   "GetOilFromChina_情况调查报告_v2.0.docx")
...
while i < len(lines):
    ...
    if "附件 C" in stripped[:30] or "参考文献" in stripped[:30] or stripped.startswith("## C-1") or stripped.startswith("## C-2"):
        in_refs = True
